              ...
              @for (item of res.data; track $index) {
                <div class="flex flex-row justify-start gap-4 bg-slate-600 text-white border-2 border-red-900 rounded shadow-lg shadow-slate-700">
                  ...
                  <div class="flex flex-col align-middle justify-start gap-4">
                    <a class="font-black shadow-rose-700 shadow-lg text-end pr-4 rounded-md mt-2" :href="{{ item.official_site }}" target="_blank">{{ item.name }}</a>
                    <p class="w-full max-w-60 text-wrap whitespace-break-spaces">{{ item.description }}</p>
                  </div>
                </div>
              }
            </div>
